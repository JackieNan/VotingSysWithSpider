...
def voters(request):
    ...
    if request.method == 'POST':
        if userForm.is_valid() and voterForm.is_valid():
            ...
            user.save()
            voter.save()
            messages.success(request, "新选民已创建")
        else:
            messages.error(request, "表格错误")
...
def updateVoter(request):
    ...
    try:
        ...
        user.save()
        voter.save()
        messages.success(request, "选民已更新")
    except:
...
def deleteVoter(request):
    ...
    try:
        ...
        admin.delete()
        messages.success(request, "选民已删除")
    except:
        messages.error(request, "拒绝访问此资源的权限")
...
def viewPositions(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            form.save()
            messages.success(request, "新职位已创建")
        else:
            messages.error(request, "表格错误")
...
def updatePosition(request):
    ...
    try:
        ...
        pos.save()
        messages.success(request, "职位已更新")
    except:
        messages.error(request, "拒绝访问此资源的权限")
...
def viewCandidates(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            form = form.save()
            messages.success(request, "已创建新候选人")
        else:
            messages.error(request, "表格错误")
...
def updateCandidate(request):
    ...
    try:
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "候选人已更新")
        else:
            messages.error(request, "表格错误")
    except:
        messages.error(request, "拒绝访问此资源的权限")
...
def deleteCandidate(request):
    ...
    try:
        ...
        pos.delete()
        messages.success(request, "候选人已删除")
    except:
        messages.error(request, "拒绝访问此资源的权限")
